...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(model.state_dict(), 'checkpoint8.pt')     # è¿™é‡Œä¼šå­˜å‚¨è¿„ä»Šæœ€ä¼˜æ¨¡åž‹çš„å‚æ•°
        # torch.save(model, 'checkpoint8.pt')                 # è¿™é‡Œä¼šå­˜å‚¨è¿„ä»Šæœ€ä¼˜çš„æ¨¡åž‹
...
def train(net,name,bs,num_epoch,device,wholemat,evalmat,LR,patience):
    ...
    for epoch in range(num_epoch):
        ...
        if epoch%100 == 0:
            torch.save(net.state_dict(), 'checkpoint8.pt')
        ...
        if math.isnan(running_loss):
            ...
            net.load_state_dict(torch.load('checkpoint8.pt')) 
    ...
    net.load_state_dict(torch.load('checkpoint8.pt')) #net=torch.load('checkpointpt')
    return net,epoch,avg_vallosses,avg_lossli
		...
		for j in range(len(functions)):
			for k in range(len(functions)):
			    if i > 8:
		   	 		...
		   	 		xv, yv, zv = (np.random.rand(100,100)-0.5)*6, (np.random.rand(100,100)-0.5)*6, (np.random.rand(100,100)-0.5)*6


		   	 		f = functions[i](xv) * functions[j](yv) * functions[k](zv)#+0.01*(np.random.random(xv.shape)-0.5)*2
		   	 		data = torch.tensor(np.vstack((np.ravel(xv), np.ravel(yv), np.ravel(zv), np.ravel(f)))).T.to(device)
		   	 		...
		   	 		torch.save(net.state_dict(), '%s/Results-UniformRandomData/%s_%s_%s_%s_%s_%s_%s_%s.pt'
								 %(loc,"NN",int(i+1),int(j+1),int(k+1),"x",seed,epochs,NNtraintime,))
