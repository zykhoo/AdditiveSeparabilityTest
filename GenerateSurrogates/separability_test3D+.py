...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(model.state_dict(), 'checkpoint9.pt')     # è¿™é‡Œä¼šå­˜å‚¨è¿„ä»Šæœ€ä¼˜æ¨¡åž‹çš„å‚æ•°
        # torch.save(model, 'checkpoint9.pt')                 # è¿™é‡Œä¼šå­˜å‚¨è¿„ä»Šæœ€ä¼˜çš„æ¨¡åž‹
...
def train(net,name,bs,num_epoch,device,wholemat,evalmat,LR,patience):
    ...
    for epoch in range(num_epoch):
        ...
        if epoch%100 == 0:
            torch.save(net.state_dict(), 'checkpoint9.pt')
        ...
        if math.isnan(running_loss):
            ...
            net.load_state_dict(torch.load('checkpoint9.pt')) 
    ...
    net.load_state_dict(torch.load('checkpoint9.pt')) #net=torch.load('checkpointpt')
    return net,epoch,avg_vallosses,avg_lossli
			...
			for k in range(len(functions)):
			    if i > 8:
		   	 		...
		   	 		torch.save(net.state_dict(), '%s/Results-UniformRandomData/%s_%s_%s_%s_%s_%s_%s_%s.pt'
								 %(loc,"NN",int(i+1),int(j+1),int(k+1),"+",seed,epochs,NNtraintime,))
